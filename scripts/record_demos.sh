...
#
# Useful overrides: DEMO_SIZE, AGG_THEME, AGG_SPEED, AGG_FONT, AGG_FONT_SIZE
# （本机 agg 默认字体栈找不到字体时，设 AGG_FONT="Source Code Pro" 之类；
# AGG_FONT_SIZE 默认 20，调大可提高 gif 清晰度）。
set -euo pipefail
cd "$(dirname "$0")/.."

IMG=docs/screenshots
OUT=/tmp/scheme-rs-demo/out
SIZE="${DEMO_SIZE:-100x26}"
THEME="${AGG_THEME:-monokai}"
SPEED="${AGG_SPEED:-1.3}"
FONT_SIZE="${AGG_FONT_SIZE:-20}"
ORDER="repl callcc stdlib macro multiline errors"
AGG="${AGG_BIN:-agg}"
FONT_ARGS=(--font-size "$FONT_SIZE")
